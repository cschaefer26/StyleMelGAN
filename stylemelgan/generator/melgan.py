...
class ResStack(nn.Module):
    def __init__(self, channel, num_layers=4):
        ...
        self.blocks = nn.ModuleList([
            nn.Sequential(
                nn.LeakyReLU(0.2),
                nn.ReflectionPad1d(get_padding(3, max(1, 3*i))),
                nn.utils.weight_norm(nn.Conv1d(channel, channel, kernel_size=3, dilation=max(1, 3*i))),
                nn.LeakyReLU(0.2),
