...
class ResStack(nn.Module):
    def __init__(self, channel, num_layers=4):
        ...
        self.blocks = nn.ModuleList([
            nn.Sequential(
                nn.LeakyReLU(0.2),
                nn.ReflectionPad1d(3**(i+1)),
                nn.utils.weight_norm(nn.Conv1d(channel, channel, kernel_size=3, dilation=3**(i+1))),
                nn.LeakyReLU(0.2),
        ...
        self.blocks_2 = nn.ModuleList([
            nn.Sequential(
                ...
                nn.utils.weight_norm(nn.Conv1d(channel, channel, kernel_size=11)),
                nn.LeakyReLU(0.2),
                nn.utils.weight_norm(nn.Conv1d(channel, channel, kernel_size=1)),
            )
            for i in range(num_layers)
    ...
    def forward(self, x):
        for shortcut, block, block2 in zip(self.shortcuts, self.blocks, self.blocks_2):
            x = shortcut(x) + block(x) + block2(x)
        return x
